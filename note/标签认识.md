# 标签的认识
https://www.jianshu.com/p/d0cc449ed667


## java中常见的配置文件标签认识：
1、<bean  id=''别名''  class="对应的类"/>:配置javaBean，即配置java对象（就是给一个类起别名对象，配置好后就可以直接只用这个别名代表这个类的对象）

2、<property  name="名字"  value="值"/>，在配置文件中定义一些变量和值

3、<map> 
                <entry key="course-2.3.0" value-ref="courseHandler"/>
     </map>,在这里面说明是个集合（是个集合范围），里面包含一些实体entry(对象相关的key和value)；

4、<import/>,在配置文件中导入相关的其他XML文件或要使用的东西；
...
xsi:schemaLocation="http://maven.apache.org/POM/4.0.0http://maven.apache.org/maven-v4_0_0.xsd">     
 ...
 <!--项目创建年份，4位数字。当产生版权信息时需要使用这个值。-->    
  ...
  <!--构建产生的所有文件存放的目录-->    
  <directory/>    
  <!--产生的构件的文件名，默认值是artifactId−artifactId−{version}。-->    
  <finalName/>    
